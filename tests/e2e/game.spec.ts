...
async function enter(page:Page,name:string,code?:string){
  ...
  if(!code){
    ...
    await expect(page.getByTestId("hero-sample").locator(".hero-added")).toHaveCount(3);
    await expect(page.getByTestId("hero-sample").locator(".hero-difference")).toHaveCount(0);
