...
export function Board({ imageUrl, differences = [], drafts = [], view, onView, tool = "move", color = "#111111", width = .008, disabled = false, onStroke, onPick, onAnswer, marks = false, persistentMarks = false, hideFound = false, label, fullViewport = false, answerPopup }: {
  ...
  const gesture = useRef<{view:View; x:number; y:number; distance:number; multi:boolean; pan:boolean} | null>(null);
  ...
  const down=(e:React.PointerEvent<HTMLDivElement>)=>{
    ...
    const temporaryPan=e.button===1;
    pointers.current.set(e.pointerId,{x:e.clientX,y:e.clientY});
    const ps=[...pointers.current.values()];
    if(ps.length===2){current.current=null;render();moved.current=true;gesture.current={view:viewRef.current,x:(ps[0]!.x+ps[1]!.x)/2,y:(ps[0]!.y+ps[1]!.y)/2,distance:Math.hypot(ps[0]!.x-ps[1]!.x,ps[0]!.y-ps[1]!.y),multi:true,pan:true};return}
  ...
  const move=(e:React.PointerEvent<HTMLDivElement>)=>{
    ...
    if(tool==="move"||g.pan){onView({...g.view,x:g.view.x+dx/r.width,y:g.view.y+dy/r.height});return}
  ...
  const up=(e:React.PointerEvent<HTMLDivElement>,cancel=false)=>{
    ...
    if(!cancel&&!g?.multi&&pointers.current.size===1){
      ...
      if(tool==="answer"&&!g?.pan&&!disabled&&!moved.current){const p=point(e);if(p.x>=0&&p.x<=1&&p.y>=0&&p.y<=1)onAnswer?.(p.x,p.y)}
    }else current.current=null;
    pointers.current.delete(e.pointerId);if(!pointers.current.size)gesture.current=null;render();
  };
  return <figure className={"image-board"+(fullViewport?" full-viewport-board":"")} style={{"--image-ratio":image.width/image.height} as CSSProperties}>{label&&<figcaption>{label}</figcaption>}
    <div ref={viewport} className={"board-viewport tool-"+tool} style={fullViewport?undefined:{aspectRatio:image.width+"/"+image.height}} onAuxClick={e=>e.preventDefault()} onPointerDown={down} onPointerMove={move} onPointerUp={e=>up(e)} onPointerCancel={e=>up(e,true)} onPointerLeave={()=>setCursor(null)}>
      <div ref={layer} className="board-layer" style={{transform:fullViewport?`translate(calc(-50% + ${view.x*100}%),calc(-50% + ${view.y*100}%)) scale(${view.zoom})`:`translate(${view.x*100}%,${view.y*100}%) scale(${view.zoom})`}}>
        <canvas ref={canvas} width={1200} height={Math.round(1200*image.height/image.width)} aria-label={label??t("drawing")}/>
        {answerPopup&&<span className={"answer-popup "+answerPopup.result} style={{left:answerPopup.x*100+"%",top:answerPopup.y*100+"%"}}><strong>{answerPopup.text}</strong>{answerPopup.subtext&&<small>{answerPopup.subtext}</small>}</span>}
      </div>
