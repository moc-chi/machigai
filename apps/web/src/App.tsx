...
function Game({snapshot,feedback,send,pending,leave,originalPreview}:{originalPreview?:string;snapshot:RoomSnapshot;feedback:AnswerFeedback|null;send:Send;pending:boolean;leave:()=>void}){
  ...
  if(snapshot.settings.deckId==="original"&&!snapshot.imageUrl)return <section className="loading"><p>{t("loading")}</p><button onClick={leave}>{t("leave")}</button></section>;
...
function Lobby({snapshot,send,pending,leave,originalPreview}:{originalPreview?:string;snapshot:RoomSnapshot;send:Send;pending:boolean;leave:()=>void}){
  ...
  const originalInput=useRef<HTMLInputElement>(null),originalSelected=snapshot.settings.deckId==="original";
  ...
  return <section className="lobby lobby-console">
    ...
    <div className="lobby-panels">
      ...
      <section className={"lobby-settings "+(mobileTab==="settings"?"active":"")}><nav className="settings-tabs"><button aria-pressed={settingsTab==="illustrations"} onClick={()=>setSettingsTab("illustrations")}>{t("deck")}</button><button aria-pressed={settingsTab==="rules"} onClick={()=>setSettingsTab("rules")}>{t("settings")}</button></nav>{settingsTab==="illustrations"?<section className="deck-card"><div className="series-scroll"><div className="series-options">{(["random","animals","people"] as const).map(series=><button key={series} disabled={!me.isHost||uploading||pending} aria-pressed={snapshot.settings.deckId===series} onClick={()=>update("deckId",series)}><strong>{t(series)}</strong>{series==="random"?<Shuffle/>:<img src={IMAGES.find(image=>image.deck===series)!.src} alt=""/>}</button>)}<button disabled={!me.isHost||uploading||pending} aria-pressed={originalSelected} onClick={()=>{if(snapshot.originalImage&&!originalSelected)update("deckId","original");else originalInput.current?.click()}}><strong>{t("originalUpload")}</strong>{originalPreview?<img src={originalPreview} alt=""/>:<ImagePlus/>}</button>{DUMMY_GENRES.map(genre=><button key={genre} className="dummy-genre" disabled aria-disabled="true"><strong>{t(genre)}</strong><ImagePlus/><small>{t("comingSoon")}</small></button>)}</div></div><input ref={originalInput} className="original-file-input" type="file" accept="image/png,image/jpeg,image/webp" onChange={e=>{const file=e.target.files?.[0];e.currentTarget.value="";if(file)void upload(file)}}/><div className="upload-status">{uploading&&<span role="status">{t("saving")}</span>}{snapshot.originalImage&&me.isHost&&<div className="original-upload"><button disabled={uploading||pending} onClick={()=>originalInput.current?.click()}><ImagePlus/>{t("changeImage")}</button><button disabled={uploading||pending} onClick={()=>void remove()}><Trash2/>{t("removeImage")}</button></div>}</div></section>:<section className="game-settings-tab">{me.isHost?<fieldset className="settings lobby-setting-controls"><label>{t("rounds")}<select value={snapshot.settings.stageCount} onChange={e=>update("stageCount",Number(e.target.value))}>{Array.from({length:10},(_,i)=><option key={i} value={i+1}>{i+1}</option>)}</select></label><label>{t("differences")}<select value={snapshot.settings.differencesPerPlayer} onChange={e=>update("differencesPerPlayer",Number(e.target.value))}>{[1,2,3,4,5].map(v=><option key={v}>{v}</option>)}</select></label>{(["drawingSeconds","answeringSeconds"] as const).map(key=><label key={key}>{t(key==="drawingSeconds"?"drawTime":"answerTime")}<select value={snapshot.settings[key]} onChange={e=>update(key,Number(e.target.value))}>{[30,45,60,90,120,180,300].map(v=><option key={v}>{t("seconds",{n:v})}</option>)}</select></label>)}</fieldset>:<dl className="setting-labels">{settingLabels.map(([label,value])=><div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl>}</section>}</section>
    </div>
    <div className="lobby-footer"><div className="invite-copy"><button onClick={()=>void copyInvite()}><Copy/>{t(copied?"copied":"invite")}</button><small>{t("roomCode")}: <strong>{snapshot.roomCode}</strong></small></div><button className="qr-button" aria-label={t("showQr")} onClick={()=>setShowQr(true)}><QrCode/></button>{me.isHost?<button className="primary start" disabled={pending||uploading||(originalSelected&&!originalPreview)||snapshot.participants.filter(p=>p.connected).length<1} onClick={()=>snapshot.participants.filter(p=>p.connected).length===1?setConfirmSolo(true):void send("game.start").catch(()=>{})}>{t("start")} →</button>:<p>{t("waiting")}</p>}</div>
...
function Drawing({snapshot,send,pending}:{snapshot:RoomSnapshot;send:Send;pending:boolean}){
  ...
  useEffect(()=>{if(snapshot.phase!=="DRAWING_FINALIZING"||autoSubmitted.current)return;autoSubmitted.current=true;const results=snapshot.settings.deckId==="original"&&snapshot.originalImage?validateOriginalSlots(snapshot.originalImage.width,snapshot.originalImage.height,slots):source.current?validateDifferenceSlots(source.current,slots):validationRef.current;const valid=slots.filter((_,index)=>results[index]?.valid).map(strokes=>({strokes}));void send("drawing.submit",{differences:valid}).catch(()=>{})},[snapshot.phase]);
...
function Results({snapshot,send,pending,leave}:{snapshot:RoomSnapshot;send:Send;pending:boolean;leave:()=>void}){
  ...
  return <section className="results"><div className="result-header"><span className="eyebrow">{t("round",{n:round.stageNo})}</span><h1>{t(isFinal?"final":"result")}</h1></div><nav className="result-tabs"><button aria-pressed={tab==="ranking"} onClick={()=>setTab("ranking")}>{t("ranking")}</button><button aria-pressed={tab==="artwork"} onClick={()=>setTab("artwork")}>{t("artwork")}</button></nav>
    <div className="result-content">{tab==="artwork"?<div className="review"><div className="gallery-tabs">{isFinal&&(snapshot.rounds??[]).map(r=><button key={r.stageNo} aria-pressed={roundNo===r.stageNo} onClick={()=>{setRoundNo(r.stageNo);setView(initialView)}}>{t("round",{n:r.stageNo})}</button>)}</div>
      ...
      <div className={"compare"+(snapshot.originalImage&&snapshot.settings.deckId==="original"&&snapshot.originalImage.height>snapshot.originalImage.width?" portrait-original":"")}><ExportBoard imageUrl={round.imageUrl} differences={[]} view={view} onView={setView} label={t("original")} filename="original.png"/><ExportBoard imageUrl={round.imageUrl} differences={shown} view={view} onView={setView} label={t("changed")} filename="spot-the-difference.png" marks={marks}/></div>
