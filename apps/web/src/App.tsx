import { SessionContext, changeOriginal, prepareOriginal, useOriginal } from "./original-image";
import { validateOriginalSlots } from "@machigai/drawing";
import { useCallback, useContext, useEffect, useRef, useState } from "react";
import { Check, Copy, Crown, Download, Eye, Hand, ImagePlus, Pencil, Pipette, QrCode, RotateCcw, Share2, Trash2, Undo2 } from "lucide-react";
...
import { participantColor } from "./participant-colors";
...
function useRoom(session:Session|null){
  ...
  const send=useCallback<Send>((type,payload={})=>{
    ...
    return new Promise<void>((resolve,reject)=>{
      ...
      socket.current!.send(JSON.stringify({type,commandId:id,gameNo:r?.gameNo,payload}));
    });
  },[]);
...
function Shell({language,onLanguage}:{language:Language;onLanguage:(value:Language)=>void}){
  ...
  const room=useRoom(session);const original=useOriginal(session,room.snapshot);
  const join=(next:Session)=>{sessionStorage.setItem(SESSION_KEY,JSON.stringify(next));setSession(next)};
  const leave=async()=>{if(room.snapshot){try{await room.send("member.leave")}catch{/* Offline exit falls back to the room expiry. */}}setConfirmLeave(false);sessionStorage.removeItem(SESSION_KEY);setSession(null);room.setError("");history.replaceState(null,"",location.pathname)};
  ...
  return <div className={"app-shell"+(gameActive?" game-active":"")}>
    ...
    <main>{!session?<Home onSession={join}/>:!room.snapshot?<section className="loading"><p>{t("loading")}</p><button onClick={requestLeave}>{t("leave")}</button></section>:<SessionContext.Provider value={session}><Game key={session.participantId} originalPreview={original.url} snapshot={original.snapshot!} feedback={room.feedback} send={room.send} pending={room.pendingCount>0} leave={requestLeave}/></SessionContext.Provider>}</main>
    ...
    {original.error&&<p className="error-toast" role="alert">{t(errorKey(original.error))}</p>}
...
function Game({snapshot,feedback,send,pending,leave,originalPreview}:{originalPreview?:string;snapshot:RoomSnapshot;feedback:AnswerFeedback|null;send:Send;pending:boolean;leave:()=>void}){
  ...
  if(snapshot.phase==="LOBBY")return <Lobby {...props} leave={leave} originalPreview={originalPreview}/>;
  if(snapshot.settings.sourceType==="original"&&!snapshot.imageUrl)return <section className="loading"><p>{t("loading")}</p><button onClick={leave}>{t("leave")}</button></section>;
  if(snapshot.phase==="DRAWING"||snapshot.phase==="DRAWING_FINALIZING")return <Drawing key={String(snapshot.gameNo)} {...props}/>;
  ...
  if(snapshot.phase==="FINAL_RESULT")return <Results {...props} leave={leave}/>;
  return <section className="loading"><h1>{t("ended")}</h1><button onClick={leave}>{t("leave")}</button></section>;
}
function difficultyTextKey(difficulty: "easy" | "normal" | "hard") { return difficulty === "easy" ? "difficultyEasy" : difficulty === "normal" ? "difficultyNormal" : "difficultyHard"; }
function Lobby({snapshot,send,pending,leave,originalPreview}:{originalPreview?:string;snapshot:RoomSnapshot;send:Send;pending:boolean;leave:()=>void}){
  ...
  const session=useContext(SessionContext)!;const [uploading,setUploading]=useState(false),[uploadError,setUploadError]=useState("");
  const [selectedImageIds,setSelectedImageIds]=useState(snapshot.settings.imageIds);const [difficulty,setDifficulty]=useState<"all"|"easy"|"normal"|"hard">("all");const originalInput=useRef<HTMLInputElement>(null);
  const originalSelected=snapshot.settings.sourceType==="original";
  const upload=async(file:File)=>{setUploading(true);setUploadError("");try{await changeOriginal(session,snapshot,await prepareOriginal(file));await send("settings.update",{sourceType:"original"})}catch(error){setUploadError(error instanceof Error?error.message:"IMAGE_INVALID")}finally{setUploading(false)}};
  const remove=async()=>{setUploading(true);setUploadError("");try{await changeOriginal(session,snapshot)}catch(error){setUploadError(error instanceof Error?error.message:"IMAGE_INVALID")}finally{setUploading(false)}};
  const update=(values:Record<string,unknown>)=>void send("settings.update",values).catch(()=>{});
  ...
  useEffect(()=>setSelectedImageIds(snapshot.settings.imageIds),[snapshot.settings.imageIds]);
  const settingLabels=[[t("differences"),t("countValue",{n:snapshot.settings.differencesPerPlayer})],[t("drawTime"),t("seconds",{n:snapshot.settings.drawingSeconds})],[t("answerTime"),t("seconds",{n:snapshot.settings.answeringSeconds})]];
  const difficultyOrder={easy:0,normal:1,hard:2} as const;
  const visibleImages=[...IMAGES].filter(image=>difficulty==="all"||image.difficulty===difficulty).sort((a,b)=>difficultyOrder[a.difficulty]-difficultyOrder[b.difficulty]);
  const selectedImageIdSet=new Set(selectedImageIds);const allVisibleSelected=visibleImages.length>0&&visibleImages.every(image=>selectedImageIdSet.has(image.id));
  const toggleImage=(id:typeof IMAGES[number]["id"])=>{if(originalSelected){update({sourceType:"standard"});return}const next=selectedImageIds.includes(id)?selectedImageIds.filter(value=>value!==id):[...selectedImageIds,id];setSelectedImageIds(next);update({imageIds:next,sourceType:"standard"})};
  const selectVisible=()=>{const visibleIds=new Set(visibleImages.map(image=>image.id));const next=allVisibleSelected?selectedImageIds.filter(id=>!visibleIds.has(id)):[...new Set([...selectedImageIds,...visibleIds])];setSelectedImageIds(next);update({imageIds:next,sourceType:"standard"})};
  return <section className="lobby lobby-console">
    ...
    <div className="lobby-panels">
      <aside className={mobileTab==="players"?"active":""}><h2>{t("members")} {snapshot.participants.length}/{snapshot.settings.maxPlayers}</h2><ul className="member-list lobby-players">{snapshot.participants.map(p=><li key={p.id}><Avatar name={p.nickname} color={participantColor(p.joinOrder)}/><strong>{p.nickname} {p.id===me.id&&"("+t("you")+")"}</strong>{p.isHost&&<span className="host-tag"><Crown/>{t("host")}</span>}{me.isHost&&!p.isHost&&<button aria-label={p.nickname+" "+t("leave")} onClick={()=>void send("member.kick",{participantId:p.id}).catch(()=>{})}>×</button>}</li>)}</ul></aside>
      <section className={"lobby-settings "+(mobileTab==="settings"?"active":"")}><nav className="settings-tabs"><button aria-pressed={settingsTab==="illustrations"} onClick={()=>setSettingsTab("illustrations")}>{t("deck")}</button><button aria-pressed={settingsTab==="rules"} onClick={()=>setSettingsTab("rules")}>{t("settings")}</button></nav>{settingsTab==="illustrations"?<section className="deck-card"><div className="image-filter"><label>{t("difficulty")}<select value={difficulty} onChange={e=>setDifficulty(e.target.value as typeof difficulty)}><option value="all">{t("all")}</option><option value="easy">{t("difficultyEasy")}</option><option value="normal">{t("difficultyNormal")}</option><option value="hard">{t("difficultyHard")}</option></select></label></div><div className="image-scroll"><div className="image-options"><button disabled={!me.isHost||uploading||pending} aria-pressed={allVisibleSelected&&!originalSelected} onClick={selectVisible}><strong>{t(allVisibleSelected?"allDeselect":"allRandom")}</strong><Check/></button><button disabled={!me.isHost||uploading||pending} aria-pressed={false} onClick={()=>originalInput.current?.click()}><strong>{t("originalUpload")}</strong><ImagePlus/></button>{snapshot.originalImage&&<button className="original-choice" disabled={!me.isHost||uploading||pending||!originalPreview} aria-pressed={originalSelected} onClick={()=>update({sourceType:originalSelected?"standard":"original"})}><strong>{t("originalUpload")}</strong>{originalPreview?<img src={originalPreview} alt=""/>:<ImagePlus/>}<small>{t("originalUpload")}</small></button>}{visibleImages.map(image=><button key={image.id} disabled={!me.isHost||uploading||pending} aria-pressed={!originalSelected&&selectedImageIds.includes(image.id)} onClick={()=>toggleImage(image.id)}><strong>{image.title}</strong><img src={image.src} alt=""/><small className={"difficulty difficulty-"+image.difficulty}>{t(difficultyTextKey(image.difficulty))}</small></button>)}</div></div><input ref={originalInput} className="original-file-input" type="file" accept="image/png,image/jpeg,image/webp" onChange={e=>{const file=e.target.files?.[0];e.currentTarget.value="";if(file)void upload(file)}}/><div className="upload-status">{uploading&&<span role="status">{t("saving")}</span>}{snapshot.originalImage&&me.isHost&&<div className="original-upload"><button disabled={uploading||pending} onClick={()=>originalInput.current?.click()}><ImagePlus/>{t("changeImage")}</button><button disabled={uploading||pending} onClick={()=>void remove()}><Trash2/>{t("removeImage")}</button></div>}</div></section>:<section className="game-settings-tab">{me.isHost?<fieldset className="settings lobby-setting-controls"><label>{t("differences")}<select value={snapshot.settings.differencesPerPlayer} onChange={e=>update({differencesPerPlayer:Number(e.target.value)})}>{[1,2,3,4,5].map(v=><option key={v} value={v}>{v}</option>)}</select></label>{(["drawingSeconds","answeringSeconds"] as const).map(key=><label key={key}>{t(key==="drawingSeconds"?"drawTime":"answerTime")}<select value={snapshot.settings[key]} onChange={e=>update({[key]:Number(e.target.value)})}>{[30,45,60,90,120,180,300].map(v=><option key={v} value={v}>{t("seconds",{n:v})}</option>)}</select></label>)}</fieldset>:<dl className="setting-labels">{settingLabels.map(([label,value])=><div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl>}</section>}</section>
    </div>
    <div className="lobby-footer"><div className="invite-copy"><button onClick={()=>void copyInvite()}><Copy/>{t(copied?"copied":"invite")}</button><small>{t("roomCode")}: <strong>{snapshot.roomCode}</strong></small></div><button className="qr-button" aria-label={t("showQr")} onClick={()=>setShowQr(true)}><QrCode/></button>{me.isHost?<button className="primary start" disabled={pending||uploading||(originalSelected?!originalPreview:!selectedImageIds.length)||snapshot.participants.filter(p=>p.connected).length<1} onClick={()=>snapshot.participants.filter(p=>p.connected).length===1?setConfirmSolo(true):void send("game.start").catch(()=>{})}>{t("start")} →</button>:<p>{t("waiting")}</p>}</div>
    ...
    {uploadError&&<div className="modal-backdrop"><section className="modal compact-modal image-error-dialog" role="alertdialog" aria-modal="true" aria-label={t("imageErrorTitle")}><h2>{t("imageErrorTitle")}</h2><p>{t(errorKey(uploadError))}</p><button autoFocus onClick={()=>setUploadError("")}>{t("close")}</button></section></div>}
  </section>;
}
function Avatar({name,color}:{name:string;color?:string}){return <span className="avatar" style={color?{backgroundColor:color}:undefined}>{Array.from(name)[0]}</span>}
...
function Drawing({snapshot,send,pending}:{snapshot:RoomSnapshot;send:Send;pending:boolean}){
  ...
  const storageKey=`drawing-${snapshot.roomId}-${snapshot.gameNo}-${snapshot.selfId}`;
  ...
  useEffect(()=>{if(snapshot.settings.sourceType==="original"&&snapshot.originalImage){setValidations(validateOriginalSlots(snapshot.originalImage.width,snapshot.originalImage.height,slotsRef.current));return}let active=true;const image=IMAGES.find(item=>item.src===snapshot.imageUrl)!;fetch(snapshot.imageUrl+".rgb").then(response=>response.arrayBuffer()).then(buffer=>{if(!active)return;source.current={width:AREA_RULES.sampleWidth,height:Math.round(AREA_RULES.sampleWidth*image.height/image.width),rgb:new Uint8Array(buffer)};setValidations(validateDifferenceSlots(source.current,slotsRef.current))}).catch(()=>{});return()=>{active=false}},[snapshot.imageUrl]);
  useEffect(()=>{if(snapshot.settings.sourceType==="original"&&snapshot.originalImage){setValidations(validateOriginalSlots(snapshot.originalImage.width,snapshot.originalImage.height,slots));return}if(!source.current)return;const timer=setTimeout(()=>setValidations(validateDifferenceSlots(source.current!,slots)),40);return()=>clearTimeout(timer)},[slots]);
  ...
  useEffect(()=>{if(snapshot.phase!=="DRAWING_FINALIZING"||autoSubmitted.current)return;autoSubmitted.current=true;const results=snapshot.settings.sourceType==="original"&&snapshot.originalImage?validateOriginalSlots(snapshot.originalImage.width,snapshot.originalImage.height,slots):source.current?validateDifferenceSlots(source.current,slots):validationRef.current;const valid=slots.filter((_,index)=>results[index]?.valid).map(strokes=>({strokes}));void send("drawing.submit",{differences:valid}).catch(()=>{})},[snapshot.phase]);
...
function Scores({snapshot,highlightWinner=false}:{snapshot:RoomSnapshot;highlightWinner?:boolean}){
  ...
  return <ol className="scores">{[...snapshot.participants].sort((a,b)=>b.score-a.score).map(p=><li key={p.id} className={highlightWinner&&p.score===highest?"winner":""}><b>{1+snapshot.participants.filter(other=>other.score>p.score).length}</b><Avatar name={p.nickname} color={participantColor(p.joinOrder)}/><span>{p.nickname}{highlightWinner&&p.score===highest&&<small className="winner-label"><Crown/>{t("winner")}</small>}</span><strong>{p.score} pt</strong></li>)}</ol>;
}
function ScoreBreakdown({snapshot}:{snapshot:RoomSnapshot}){
  const t=useText();const scores=snapshot.scores;
...
function Results({snapshot,send,pending,leave}:{snapshot:RoomSnapshot;send:Send;pending:boolean;leave:()=>void}){
  const t=useText();const [filter,setFilter]=useState("all");const [view,setView]=useState<View>(initialView);const [marks,setMarks]=useState(true);
  const [tab,setTab]=useState<"ranking"|"artwork">("ranking");const me=snapshot.participants.find(p=>p.id===snapshot.selfId)!;
  const shown=filter==="all"?snapshot.differences:snapshot.differences.filter(d=>d.creatorId===filter);
  const markColors=Object.fromEntries(snapshot.participants.map(p=>[p.id,participantColor(p.joinOrder)]));
  const imageSize=snapshot.settings.sourceType==="original"?snapshot.originalImage:undefined;
  return <section className="results"><div className="result-header"><h1>{t("final")}</h1></div><nav className="result-tabs"><button aria-pressed={tab==="ranking"} onClick={()=>setTab("ranking")}>{t("ranking")}</button><button aria-pressed={tab==="artwork"} onClick={()=>setTab("artwork")}>{t("artwork")}</button></nav>
    <div className="result-content">{tab==="artwork"?<div className="review">
      ...
      <div className="gallery-tabs"><button aria-pressed={filter==="all"} onClick={()=>setFilter("all")}>{t("all")}</button>{snapshot.participants.map(p=><button key={p.id} aria-pressed={filter===p.id} onClick={()=>setFilter(p.id)}><i className="participant-color" style={{backgroundColor:participantColor(p.joinOrder)}}/>{p.nickname}</button>)}</div>
      <div className="compare"><ExportBoard imageUrl={snapshot.imageUrl} imageSize={imageSize} differences={[]} view={view} onView={setView} label={t("original")} filename="original.png"/><ExportBoard imageUrl={snapshot.imageUrl} imageSize={imageSize} differences={shown} view={view} onView={setView} label={t("changed")} filename="spot-the-difference.png" marks={marks} markColors={markColors}/></div>
      <SharePanel imageUrl={snapshot.imageUrl} differences={shown}/>
    </div>:<div className="ranking-panel"><Scores snapshot={snapshot} highlightWinner/><ScoreBreakdown snapshot={snapshot}/><SharePanel imageUrl={snapshot.imageUrl} differences={shown}/></div>}</div><div className="result-actions">{me.isHost?<button className="primary" disabled={pending} onClick={()=>void send("game.rematch").catch(()=>{})}>{t("rematch")}</button>:<p>{t("waiting")}</p>}<button onClick={leave}>{t("leave")}</button></div>
  </section>;
}
function ExportBoard({imageUrl,imageSize,differences,view,onView,label,filename,marks=false,markColors}:{imageUrl:string;imageSize?:{width:number;height:number};differences:Difference[];view:View;onView:(view:View)=>void;label:string;filename:string;marks?:boolean;markColors?:Record<string,string>}){
...
function SharePanel({imageUrl,differences}:{imageUrl:string;differences:Difference[]}){
  ...
  const share=async()=>{if(!blob)return;const file=new File([blob],"difference-party.png",{type:"image/png"});
    try{if(navigator.canShare?.({files:[file]}))await navigator.share({files:[file],title:t("app"),text:t("shareText"),url:shareUrl});else downloadImage(blob)}catch(error){if(!(error instanceof DOMException&&error.name==="AbortError"))setError(true)}
  };
  ...
  const homeUrl=location.origin+location.pathname;const shareUrl=homeUrl;const xUrl="https://twitter.com/intent/tweet?text="+encodeURIComponent(t("shareText"))+"&url="+encodeURIComponent(homeUrl);
