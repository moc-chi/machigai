...
function Lobby({snapshot,send,pending,leave,originalPreview}:{originalPreview?:string;snapshot:RoomSnapshot;send:Send;pending:boolean;leave:()=>void}){
  ...
  const upload=async(file:File)=>{setUploading(true);setUploadError("");try{await changeOriginal(session,snapshot,await prepareOriginal(file))}catch(error){setUploadError(error instanceof Error?error.message:"IMAGE_INVALID")}finally{setUploading(false)}};
...
function Results({snapshot,send,pending,leave}:{snapshot:RoomSnapshot;send:Send;pending:boolean;leave:()=>void}){
  ...
  return <section className="results"><div className="result-header"><h1>{t("final")}</h1></div><nav className="result-tabs"><button aria-pressed={tab==="ranking"} onClick={()=>setTab("ranking")}>{t("ranking")}</button><button aria-pressed={tab==="artwork"} onClick={()=>setTab("artwork")}>{t("artwork")}</button></nav>
    <div className="result-content">{tab==="artwork"?<div className="review">
      ...
      <div className={"compare"+(snapshot.originalImage&&snapshot.settings.sourceType==="original"&&snapshot.originalImage.height>snapshot.originalImage.width?" portrait-original":"")}><ExportBoard imageUrl={snapshot.imageUrl} differences={[]} view={view} onView={setView} label={t("original")} filename="original.png"/><ExportBoard imageUrl={snapshot.imageUrl} differences={shown} view={view} onView={setView} label={t("changed")} filename="spot-the-difference.png" marks={marks}/></div>
      <SharePanel imageUrl={snapshot.imageUrl} differences={shown}/>
    </div>:<div className="ranking-panel"><Scores snapshot={snapshot} highlightWinner/><ScoreBreakdown snapshot={snapshot}/></div>}</div><div className="result-actions">{me.isHost?<button className="primary" disabled={pending} onClick={()=>void send("game.rematch").catch(()=>{})}>{t("rematch")}</button>:<p>{t("waiting")}</p>}<button onClick={leave}>{t("leave")}</button></div>
  </section>;
}
