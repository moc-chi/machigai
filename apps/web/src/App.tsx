...
function Home({onSession}:{onSession:(session:Session)=>void}){
  ...
  return <section className="home"><div><p className="eyebrow">1–10 PLAYERS</p><h1>{t("homeTitle")}</h1><p className="lead">{t("homeLead")}</p><div className="home-actions"><button className="primary" onClick={()=>setMode("create")}>{t("create")} →</button><button onClick={()=>setMode("join")}>{t("join")}</button></div></div><div className="hero-sample" data-testid="hero-sample">
...
function Lobby({snapshot,send,pending,leave}:{snapshot:RoomSnapshot;send:Send;pending:boolean;leave:()=>void}){
  ...
  const [original,setOriginal]=useState<{name:string;url:string}|null>(null);const [originalSelected,setOriginalSelected]=useState(false);const [selectedImageIds,setSelectedImageIds]=useState(snapshot.settings.imageIds);const [difficulty,setDifficulty]=useState<"all"|"easy"|"normal"|"hard">("all");const originalInput=useRef<HTMLInputElement>(null);
  useEffect(()=>()=>{if(original)URL.revokeObjectURL(original.url)},[original]);
  const update=(key:string,value:unknown)=>void send("settings.update",{[key]:value}).catch(()=>{});
  ...
  const allVisibleSelected=visibleImages.length>0&&visibleImages.every(image=>selectedImageIdSet.has(image.id));
  const toggleImage=(id:typeof IMAGES[number]["id"])=>{const next=selectedImageIds.includes(id)?selectedImageIds.filter(value=>value!==id):[...selectedImageIds,id];setSelectedImageIds(next);update("imageIds",next)};
  const selectVisible=()=>{const visibleIds=new Set(visibleImages.map(image=>image.id));const next=allVisibleSelected?selectedImageIds.filter(id=>!visibleIds.has(id)):[...new Set([...selectedImageIds,...visibleIds])];setSelectedImageIds(next);update("imageIds",next)};
  return <section className="lobby lobby-console">
    ...
    <div className="lobby-panels">
      ...
      <section className={"lobby-settings "+(mobileTab==="settings"?"active":"")}><nav className="settings-tabs"><button aria-pressed={settingsTab==="illustrations"} onClick={()=>setSettingsTab("illustrations")}>{t("deck")}</button><button aria-pressed={settingsTab==="rules"} onClick={()=>setSettingsTab("rules")}>{t("settings")}</button></nav>{settingsTab==="illustrations"?<section className="deck-card"><div className="image-filter"><label>{t("difficulty")}<select value={difficulty} onChange={e=>setDifficulty(e.target.value as typeof difficulty)}><option value="all">{t("all")}</option><option value="easy">{t("difficultyEasy")}</option><option value="normal">{t("difficultyNormal")}</option><option value="hard">{t("difficultyHard")}</option></select></label></div><div className="image-scroll"><div className="image-options"><button disabled={!me.isHost} aria-pressed={allVisibleSelected} onClick={selectVisible}><strong>{t(allVisibleSelected?"allDeselect":"allRandom")}</strong><Shuffle/></button><button disabled={!me.isHost} aria-pressed={false} onClick={()=>originalInput.current?.click()}><strong>{t("originalUpload")}</strong><ImagePlus/></button>{original&&<button className="original-choice" aria-pressed={originalSelected} onClick={()=>setOriginalSelected(value=>!value)}><strong>{original.name}</strong><img src={original.url} alt=""/><small>{t("originalUpload")}</small></button>}{visibleImages.map(image=><button key={image.id} disabled={!me.isHost} aria-pressed={selectedImageIds.includes(image.id)} onClick={()=>toggleImage(image.id)}><strong>{image.title}</strong><img src={image.src} alt=""/><small className={"difficulty difficulty-"+image.difficulty}>{t(difficultyTextKey(image.difficulty))}</small></button>)}</div></div><input ref={originalInput} className="original-file-input" type="file" accept="image/png,image/jpeg,image/webp" onChange={e=>{const file=e.target.files?.[0];if(!file)return;setOriginal(previous=>{if(previous)URL.revokeObjectURL(previous.url);return{name:file.name,url:URL.createObjectURL(file)}});setOriginalSelected(true);e.currentTarget.value=""}}/></section>:<section className="game-settings-tab">{me.isHost?<fieldset className="settings lobby-setting-controls"><label>{t("differences")}<select value={snapshot.settings.differencesPerPlayer} onChange={e=>update("differencesPerPlayer",Number(e.target.value))}>{[1,2,3,4,5].map(v=><option key={v}>{v}</option>)}</select></label>{(["drawingSeconds","answeringSeconds"] as const).map(key=><label key={key}>{t(key==="drawingSeconds"?"drawTime":"answerTime")}<select value={snapshot.settings[key]} onChange={e=>update(key,Number(e.target.value))}>{[30,45,60,90,120,180,300].map(v=><option key={v}>{t("seconds",{n:v})}</option>)}</select></label>)}</fieldset>:<dl className="setting-labels">{settingLabels.map(([label,value])=><div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl>}</section>}</section>
    </div>
    <div className="lobby-footer"><div className="invite-copy"><button onClick={()=>void copyInvite()}><Copy/>{t(copied?"copied":"invite")}</button><small>{t("roomCode")}: <strong>{snapshot.roomCode}</strong></small></div><button className="qr-button" aria-label={t("showQr")} onClick={()=>setShowQr(true)}><QrCode/></button>{me.isHost?<button className="primary start" disabled={pending||!selectedImageIds.length||snapshot.participants.filter(p=>p.connected).length<1} onClick={()=>snapshot.participants.filter(p=>p.connected).length===1?setConfirmSolo(true):void send("game.start").catch(()=>{})}>{t("start")} →</button>:<p>{t("waiting")}</p>}</div>
...
function Results({snapshot,send,pending,leave}:{snapshot:RoomSnapshot;send:Send;pending:boolean;leave:()=>void}){
  ...
  const [tab,setTab]=useState<"ranking"|"artwork">("ranking");
  ...
  return <section className="results"><div className="result-header"><h1>{t("final")}</h1></div><nav className="result-tabs"><button aria-pressed={tab==="ranking"} onClick={()=>setTab("ranking")}>{t("ranking")}</button><button aria-pressed={tab==="artwork"} onClick={()=>setTab("artwork")}>{t("artwork")}</button></nav>
    <div className="result-content">{tab==="artwork"?<div className="review">
      ...
      <div className="compare"><ExportBoard imageUrl={snapshot.imageUrl} differences={[]} view={view} onView={setView} label={t("original")} filename="original.png"/><ExportBoard imageUrl={snapshot.imageUrl} differences={shown} view={view} onView={setView} label={t("changed")} filename="spot-the-difference.png" marks={marks}/></div>
