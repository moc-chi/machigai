...
function Lobby({snapshot,send,pending,leave,originalPreview}:{originalPreview?:string;snapshot:RoomSnapshot;send:Send;pending:boolean;leave:()=>void}){
  ...
  const toggleImage=(id:typeof IMAGES[number]["id"])=>{const next=selectedImageIds.includes(id)?selectedImageIds.filter(value=>value!==id):[...selectedImageIds,id];setSelectedImageIds(next);update({imageIds:next,sourceType:"standard"})};
