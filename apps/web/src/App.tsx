...
function Home({onSession}:{onSession:(session:Session)=>void}){
  ...
  return <section className="home"><div><p className="eyebrow">2–10 PLAYERS</p><h1>{t("homeTitle")}</h1><p className="lead">{t("homeLead")}</p><div className="home-actions"><button className="primary" onClick={()=>setMode("create")}>{t("create")} →</button><button onClick={()=>setMode("join")}>{t("join")}</button></div></div><div className="hero-sample" data-testid="hero-sample">
    <figure><figcaption>{t("original")}</figcaption><img src="/assets/bakery.png" alt={t("original")}/></figure>
    <figure><figcaption>{t("changed")}</figcaption><div className="hero-changed"><img src="/assets/bakery.png" alt={t("changed")}/><svg viewBox="0 0 100 66.67" aria-hidden="true"><path className="hero-difference coral" d="M19 26c2-5 6-5 8 0m-7-2 6 0"/><path className="hero-difference blue" d="M64 47c3-4 6-4 9 0m-8 0c2 3 5 3 7 0"/><path className="hero-difference yellow" d="M81 19l2 4 5 .5-4 3 1 5-4-2-4 2 1-5-4-3 5-.5z"/></svg></div></figure>
  </div>
